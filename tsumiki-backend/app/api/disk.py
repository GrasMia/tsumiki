...
async def move(
    ...
    target_path = validate_dir_path(target_path)
    original_path = validate_dir_path(original_path)

    if dir_name and not file_name:
        dir_name = validate_dir_name(dir_name)

        target_dir = await db.scalar(select(Dir).where(Dir.path == (f"/{target_path}/")).with_for_update())
        original_dir = await db.scalar(select(Dir).where(Dir.path == (f"/{original_path}/")).with_for_update())
        ...
        if target_dir.id == original_dir.id:
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="禁止原地TP")
        ...
        return {"detail": f"文件 {file_name} 已移动至 {target_dir.name} 目录下"}
